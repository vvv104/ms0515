...
| `verdict.py` | Shared confidence model (bands + `base_disk` physical-disk grouping + `classify`) used by `report.py` and `export.py`.  A physical disk merges all captures of one floppy, so re-reads of one disk never count as the cross-disk GUARANTEED bar. |
