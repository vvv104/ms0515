...
- Runs in the browser too - <https://vvv104.github.io/ms0515/>: the core
  ...
  disks the guest writes to stay in the browser's storage, and "Bug report"
  saves a file with the machine's state, the ROM and the mounted images -
...
```bash
cd package
ms0515.exe --disk0 path/to/disk.dsk
```
...
| `--disk0 <path>` | `-d0` | Drive 0 by the image's size: a 819200-byte track-interleaved DS image both sides, a 409600-byte SS image the lower side |
...
a second SS image on the drive.
