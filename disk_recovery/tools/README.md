...
| `review.py` | Desktop GUI (tkinter, stdlib) to review recoveries and pick canonical versions.  Tabs = confidence bands; each lists its files; selecting one shows its versions and the disks each is on, with **View** (KOI-8R text or hex), **Diff 2** (unified text / block-hex diff), **Set canonical** (writes the pick to `decisions.tsv` → file moves to CHOSEN) and **Clear**.  Reads `decisions.tsv` on start so already-decided files are out of AMBIGUOUS.  `python review.py` (or `--selftest` for a headless model check). |
