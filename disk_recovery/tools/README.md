...
| `report.py` | Turn `consensus.json` (+ `donor.json`) into a per-file **recovery-confidence matrix**: each file banded HIGH (>=2 captures identical) / GOOD (reconciled or donor-recovered) / CORROBORATED (single + a matching 2nd copy) / MEDIUM (single, every sector CRC-clean) / UNVERIFIED (single raw, no way to check) / AMBIGUOUS (several builds) / LOST (bad on every copy), with the action needed.  Emits the **healthy** set (HIGH/GOOD/CORROBORATED/MEDIUM — no corruption + a positive corroboration).  Writes `work/corpus/REPORT.md`, `work/corpus/report.csv` (full matrix), and `work/corpus/healthy.txt` (the trustworthy files). |
