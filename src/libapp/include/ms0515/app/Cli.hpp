...
struct CliArgs {
    ...
    std::string dsPath[2];
    /* HD: paravirtual hard disk.  hdEnabled presents the controller on the
     * bus (mutually exclusive with the serial port); hdPath is the image /
     * media (a non-empty path also implies an enabled controller).  Any
