...
51$:    MOVB    (R0)+,(R2)+
        ...
        BNE     51$
        CLRB    (R2)
        INC     R5                     ; n = runlen+1 bytes
        MOV     #W8B02,R2
        CLR     R0                     ; RRD carry nibble (A.lo << 4), starts 0
6$:     MOVB    (R2),R1                ; the RRD pass, table-driven: t = NIBSW[v]
        BIC     #177400,R1             ;   (v's nibbles swapped), so v>>4 = t & $0F
        MOVB    NIBSW(R1),R1           ;   and (v & $0F) << 4 = t & $F0
        BIC     #177400,R1
        MOV     R1,R3
        BIC     #177760,R3
        BIS     R0,R3                  ; m = (v >> 4) | carry
        MOVB    R3,(R2)+
        MOV     R1,R0                  ; carry = (v & $0F) << 4
        BIC     #177417,R0
        DEC     R5
        BNE     6$
        MOVB    -(R2),R0
...
C407M:  .BYTE   %C407%.                ; facing flag ($C407)
        .EVEN
W:      .BLKB   64.
NIBSW:                                 ; nibble-swap table: the Z80 RRD pass in one lookup
