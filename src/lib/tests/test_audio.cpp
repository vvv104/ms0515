...
TEST_CASE("a move reads the full stroke between the tracks it crosses") {
    auto d = std::make_shared<DriveSounds>();
    /* 8000 samples for 79 tracks: about a hundred a track, and each sample
     * says where in the stroke it is, so what is played can be read off it. */
    d->strokeIn.rate = kRate;
    d->strokeOut.rate = kRate;
    for (int i = 0; i < 8000; ++i) {
        d->strokeIn.samples.push_back(static_cast<int16_t>(i / 4 + 1));
        d->strokeOut.samples.push_back(static_cast<int16_t>(-(i / 4 + 1)));
    }
    const int past = kRate / 100;               /* 10 ms, past the way in */
    AudioRenderer r(kRate);
    r.setSpeakerVolume(0.0f);
    r.setDriveSounds(d);

    r.seek(0, 30, 3000);                        /* out of track 0, 30 tracks in */
    auto out = frame(r);
    const int near = std::abs(out[past]);
    CHECK(near > 0);
    CHECK(near < 600);                          /* the early part of the stroke */

    for (int i = 0; i < 6; ++i) (void)frame(r);   /* let that move finish */

    r.seek(0, 30, 3000);                        /* on from there, another 30 */
    out = frame(r);
    const int far = std::abs(out[past]);
    CHECK(far > near + 200);                    /* further along the same recording */
    for (int i = 0; i < 6; ++i) (void)frame(r);

    r.seek(0, -60, 3000);                       /* and all the way home */
    out = frame(r);
    CHECK(out[past] < 0);                       /* the outward recording */
}

TEST_CASE("one step: the note rises with the track the head stands on") {
    auto d = std::make_shared<DriveSounds>();
    d->stepIn = flat(kRate, 400, 1000);
    d->stepOut = flat(kRate, 400, 1000);
    AudioRenderer r(kRate);
    r.setSpeakerVolume(0.0f);
    r.setDriveSounds(d);

    /* A step at track 0 plays the recording as it is; by the last track it is
     * read fast enough to be over sooner - the note a shade higher, which is
     * what a formatting run sounds like from one end of the disc to the other. */
    r.seek(0, 1, 3000);
    auto at0 = frame(r);
    int first = 0;
    while (first < 400 && at0[static_cast<std::size_t>(first)] != 0) ++first;

    for (int i = 0; i < 77; ++i) { r.seek(0, 1, 3000); (void)frame(r); }
    r.seek(0, 1, 3000);
    auto atEnd = frame(r);
    int last = 0;
    while (last < 400 && atEnd[static_cast<std::size_t>(last)] != 0) ++last;

    CHECK(last < first);                        /* read faster, so it ends sooner */
    CHECK(first - last > 10);
}
