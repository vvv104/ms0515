...
| `ms_disk_booted(path, side)` / `ms_disk_system(path, side, fromPath, fromSide)` + `ms_disk_text()` | the monitor a floppy side boots; a side made a system volume of another's - the kit copied, the bootstrap written as COPY/BOOT does |
...
disk (`ms_disk_system`): the kit - every .SYS, PIP, DUP, DIR, RESORC -
copied over and the bootstrap written as RT-11's COPY/BOOT writes it
