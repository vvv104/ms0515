/*
 * Config.cpp — YAML config loader / writer.
 */
...
void Config::save() const
{
    ...
    /* Double-sided drives take precedence; the side-N fields below
     * are NOT also written when a DS path is set for the same drive. */
                       ...
                       const std::string &cfgRomPath)
{
    ...
    std::error_code ec;
    if (!cliRomPath.empty() && fs::exists(cliRomPath, ec))
        return cliRomPath;
    if (!cfgRomPath.empty() && fs::exists(cfgRomPath, ec))
        return cfgRomPath;
    return Paths::findAssetRom("ms0515-roma.rom");
}
